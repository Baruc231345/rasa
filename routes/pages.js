...
router.get("/verification2/:hashedId", async (req, res) => {
  ...
  const html = `
    <h1>Rasa for Approval Email</h1>
    <a href="http://localhost:3005/getSignature2/${hashedId}" style="background-color: green; color: white; padding: 10px; text-decoration: none;">Approve</a>
  `;
...
router.get("/getSignature/:id", async (req, res) => {
  ...
  db1.query(checkFormSignQuery, [decryptedrasaID], async (error, result) => {
    ...
    const updateQuery =
      "UPDATE inputted_table SET form_sign = (SELECT form_sign FROM signature_table2 WHERE id = 1) WHERE id = ?";
